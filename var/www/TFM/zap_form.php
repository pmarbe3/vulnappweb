...
<head>
  <meta charset="UTF-8">
  <title>Escaneo de Vulnerabilidades - ZAP</title>
  <style>
    :root {
      --uoc-blue: #0099cc;
      --uoc-gray: #f2f2f2;
...
<body>
  ...
  <header>
    ...
    <h1>Escaneo de Vulnerabilidades con ZAP</h1>
  </header>

  <main>
    <form action="zap_handler.php" method="post">
      <label for="url">Introduce la URL o IP a analizar (ej. http://192.168.1.101:80):</label>
      <input type="text" id="url" name="url" required pattern="^https?:\/\/.+" 
             title="Debes introducir una URL que comience por http:// o https://">
